/*
  1:歌曲搜索接口
    请求地址:https://netyicloudmusicapi.liuyifei.tech/search
    请求方法:get
    请求参数:keywords(查询关键字)
    响应内容:歌曲搜索结果
  2:歌曲url获取接口
    请求地址:https://netyicloudmusicapi.liuyifei.tech/song/url
    请求方法:get
    请求参数:id(歌曲id)
    响应内容:歌曲url地址
  3.歌曲详情获取
    请求地址:https://netyicloudmusicapi.liuyifei.tech/song/detail
    请求方法:get
    请求参数:ids(歌曲id)
    响应内容:歌曲详情(包括封面信息)
  4.热门评论获取
    请求地址:https://netyicloudmusicapi.liuyifei.tech/comment/hot?type=0
    请求方法:get
    请求参数:id(歌曲id,地址中的type固定为0)
    响应内容:歌曲的热门评论
  5.mv地址获取
    请求地址:https://netyicloudmusicapi.liuyifei.tech/mv/url
    请求方法:get
...
var app = new Vue({
    ...
    methods: {
        searchMusic: function () {
            var that = this;
            axios.get("https://netyicloudmusicapi.liuyifei.tech/cloudsearch?keywords=" + this.query)
                .then(
                    function (response) {
        ...
        playMusic(id) {
            ...
            axios.get("https://netyicloudmusicapi.liuyifei.tech/song/detail?ids=" + id)
                .then(function (response) {
                    that.musicCover = response.data.songs[0].al.picUrl;
                }, function (err) { });
            axios.get('https://netyicloudmusicapi.liuyifei.tech/comment/hot?type=0&id=' + id)
                .then(function (response) {
                    that.musicComment = response.data.hotComments;
                }, function (err) { });
            axios.get('https://netyicloudmusicapi.liuyifei.tech/lyric?id=' + id)
                .then(function(response){
        ...
        playMV(mvid) {
            var that = this;
            this.isShow = true;
            axios.get("https://netyicloudmusicapi.liuyifei.tech/mv/url?id=" + mvid)
                .then(function (response) {
